...
export default (io: Server) => {
  io.on('connection', (socket) => {
    socket.on('joinRoom', (_data: any) => {
      ...
      const { roomIdx } = JSON.parse(_data);
      socket.join(`room:${roomIdx}`);
    });

    socket.on('editFile', async (_data: any) => {
      if (socket.request.session?.user) {
        const { event, roomIdx, data } = JSON.parse(_data);
    ...
    socket.on('openFile', async (_data: any) => {
      for (const room of socket.rooms) {
      ...
      const { fileIdx } = JSON.parse(_data);
    ...
    socket.on('pushCode', async (_data: any) => {
      if (socket.request.session?.user) {
        const { fileIdx, code } = JSON.parse(_data);
        try {
